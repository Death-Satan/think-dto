...
use think\route\annotation\BaseAnnotation;
...
class Controller extends Dispatch
{
    ...
    public function exec()
    {
        ...
        return $this->app->middleware->pipeline('controller')
            ...
            ->then(function () use ($instance) {
                ...
                // DTO 处理
                $vars = $this->handle_dto($reflect->getParameters(),$vars);
    ...
    /**
     * DTO处理
     * @param  ReflectionParameter[]  $reflect_params
     * @param array $vars
     * @return array
     */
    protected function handle_dto(array $reflect_params = [],array $vars = []):array
    {
        foreach ($reflect_params as $param)
        {
            ...
            $class_name = $reflectionType->getName();
            // 获取到类实例
            $dto_object = $this->app->make($class_name);

            $attributes = $param->getAttributes();
            foreach ($attributes as $attribute)
            {
                $attribute_class_name = $attribute->getName();
                if (!$this->app->exists($attribute_class_name))
                {
                    $attribute_object = $this->app->get($attribute_class_name);
                    if ($attribute_object instanceof BaseAnnotation)
                    {
                        $dto_object = $attribute_object->handle($dto_object);
                    }
                }
            }
            $vars[$name] = $dto_object;
            return $vars;
        }
        return $vars;
    }
